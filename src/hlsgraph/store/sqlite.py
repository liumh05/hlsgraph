...
class LedgerStore:
    ...
    @staticmethod
    def _require_real_tool_run(run: ToolRun, *, stage: str, label: str) -> None:
    ...
    def _require_typed_verification_evidence(
        ...
        cls._require_real_tool_run(
            run, stage=run_stage, label=f"verification {item.id}",
        )
    ...
    def _require_physical_gate_evidence(
        ...
        run = ToolRun.from_dict(json.loads(row[0]))
        cls._require_real_tool_run(
            run, stage="post_route", label=f"physical gate {item.id}",
        )
    ...
    def load_graph(self, snapshot_id: str) -> CanonicalGraph:
        with self.read() as connection:
            ...
            if view[0] != SCHEMA_VERSION:
                raise StoreError(
                    f"graph schema {view[0]!r} is not supported by this build ({SCHEMA_VERSION!r})"
                )
    ...
    def _set_active_snapshot(
        ...
        if view[0] != SCHEMA_VERSION:
            raise StoreError(
                ...
                f"({SCHEMA_VERSION!r}); refusing to activate it"
            )
        connection.execute(
